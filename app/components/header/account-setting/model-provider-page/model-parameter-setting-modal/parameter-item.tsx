...
const ParameterItem: FC<ParameterItemProps> = ({
  ...
  const renderInput = () => {
    ...
    if (parameterRule.type === 'int') {
      ...
      return (
        <>
          {numberInputWithSlide && false && <Slider
            className='w-[120px]'
          ...
          <input
            ref={numberInputRef}
            className='shrink-0 block pl-3 w-36 h-8 appearance-none outline-none rounded-lg bg-gray-100 text-[13px] text-gra-900'
            type='number'
            max={parameterRule.max}
            min={parameterRule.min}
  ...
  return (
    <div className={`flex items-center justify-between ${className}`}>
      <div>
        <div className={cn(isInWorkflow ? 'w-[300px]' : 'w-full', 'ml-4 shrink-0 flex items-center')}>
