const translation = {
  ...
  blocks: {
    ...
    'http-request': 'API请求',
    'variable-assigner': '变量设定器',
    'variable-aggregator': '变量聚合',
    'assigner': '变量设定',
    'iteration-start': '循环开始',
    'iteration': '循环执行',
  ...
  blocksAbout: {
    ...
    'end': '工作流执行结束节点，并输出对应执行结果。',
    'answer': '定义一个聊天对话的回复内容',
    'llm': '调用大语言或多模态大模型，使用提示词、引用变量和知识库，生成回复。',
    'knowledge-retrieval': '根据输入的参数变量，在知识库中召回最匹配的信息。',
    'question-classifier': '根据用户的输入识别其意图，依据提示词并调用大模型，匹配预设的意图分类。',
    'if-else': '根据节点的输入变量，是否满足预设条件，进入对应的工作流分支执行。',
    'code': '编写代码实现输入变量根据自定义逻辑产生返回值。',
    'template-transform': '使用 Jinja 模板语法将数据转换为字符串',
    'http-request': '通过Http发送API请求，从接口返回数据。',
    'variable-assigner': '将多路分支的变量聚合为一个变量，以实现下游节点统一配置。',
    'assigner': '将上一个节点输出变量，赋值到一个新的变量中。',
    'variable-aggregator': '将多个变量聚合成一个变量，便于下面节点调度。',
    'iteration': '循环执行内部子工作流，直到满足退出条件。',
    'parameter-extractor': '利用 LLM 从自然语言内推理提取出结构化参数，用于后置的工具调用或 HTTP 请求。',
    'document-extractor': '通过大语言模型或多模态大模型，将用户上传的文档，生成返回值。',
    'list-operator': '用于过滤或排序数组内容。',
    'toolbox': '根据功能需要，调用工具库中的工具。',
    'workflow': '调用已发布的智能体，嵌入到工作流节点中。',
    'note': '添加注释',
  },
  operator: {
    zoomIn: '放大',
  ...
  nodes: {
    ...
    llm: {
      ...
      addMessage: '添加提示词',
      roleDescription: {
        system: '可为对话提供系统级指导，如设定人设和回复逻辑。在提示词中支持使用 {{变量名} 引用输入变量。',
        user: '向模型提供用户指令，如查询或任何基于文本输入的提问。',
